...
app.post('/api/chat', async (req, res) => {
  ...
  try {
    ...
    const response = await client.messages.create({ model: 'claude-sonnet-4-6', max_tokens: 256, system, messages: [{ role: 'user', content: userContent }] });
...
[{"text":"<one precise declarative sentence>","isTrue":true or false,"category":"${cat}","hint":"<a subtle nudge>","explanation":"<1-2 sentences>"}]`;
  try {
    const client = new Anthropic({ apiKey });
    const resp = await client.messages.create({ model: 'claude-sonnet-4-5', max_tokens: 2800, system, messages: [{ role: 'user', content: 'Generate the mixed facts array now.' }] });
  ...
  } catch (err) { console.error(`[facts] batch error for ${cat}:`, err.message); }
  finally { generating[cat] = false; }
}
...
{"text":"<one precise declarative sentence>","isTrue":${mustBeTrue},"category":"${cat}","hint":"<a very subtle nudge>","explanation":"<1-2 sentences>"}`;
    try {
      const client = new Anthropic({ apiKey });
      const resp = await client.messages.create({ model: 'claude-sonnet-4-5', max_tokens: 350, system, messages: [{ role: 'user', content: 'Generate the hard final question now.' }] });
      const raw = (resp.content?.find(b => b.type === 'text')?.text || '').trim().replace(/^```json?\n?/, '').replace(/\n?```$/, '');
...
- NEVER use bold text or markdown. No filler phrases.`;
  ...
  try {
    const client = new Anthropic({ apiKey });
    const response = await client.messages.create({ model: 'claude-haiku-4-5', max_tokens: 100, system, messages });
    const block = response.content?.find(b => b.type === 'text');
